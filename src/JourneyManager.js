...
class JourneyManager {
    ...
    async greetingsJourney() {
        let messages = await prompt.setJourney("greetings");

        let steps = [
            new Step(null, messages.cat_readme_md, null),
            new Step(null, messages.title_welcome, null),
            new Step(null, messages.learn_bloom_filters, null),
            new Step(null, messages.instruction_resize_add_dummy, null),
            new Step(null, messages.instruction_add_check, null),
            new Step(null, messages.instruction_disable_step_by_step, null),
            new Step(null, messages.cta_get_started, null),
            new Step(prompt.newLine(), null, null)
        ];

        let journey = new Journey("greetings", {}, steps);
        await journey.startJourney();
    }
}

export default JourneyManager;
export { JourneyManager };